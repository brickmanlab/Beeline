#!/bin/bash
## specify $1 for input *.csv file, $2 for number of parallel jobs, $3 for trajectory_file , $4 for cell_select_file, $5 for historyLength, $6 for output gene matrix file.
...
mpirun_cmd='time mpirun --allow-run-as-root'
if [ -d "outputs" ]; then
	rm -rf outputs
fi
mkdir outputs
...
do
	...
	#echo $mpirun_cmd
	mpirun_cmd=`echo $mpirun_cmd | sed -e "s%$% -np 1 python3 runTE.py pair_jobs\/${input_file} outputs\/TE_out_${output_id}.csv $3 $4 $5 :%g"`
	#echo $mpirun_cmd
done
...
python3 makeTEasMatrix.py $6
rm TE_result_all.csv

# $6 output file
# $7 pval-cutoff
# python3 makeGRN.py $6 $7