...
def generateInputs(RunnerObj):
    ...
    counts = pd.read_csv(RunnerObj.inputDir.joinpath(RunnerObj.exprData), index_col=0)
    pseudotime = pd.read_csv(RunnerObj.inputDir.joinpath(RunnerObj.cellData), index_col=0)

    datasets = pseudotime.columns
    for idx, column in enumerate(datasets):
        metadata = pd.DataFrame(pseudotime[column].to_numpy(), columns = ['pseudotime'])
        metadata.index = counts.columns
        adata = anndata.AnnData(X=counts.T, obs=metadata)
        adata = adata[adata.obs.dropna().index]
        
        if str(RunnerObj.params['scale']) == 'True':
            sc.pp.scale(adata)
        
        adata.write(f'{RunnerObj.inputDir.joinpath("CCMNET")}/data-{idx}.h5ad')
...
def run(RunnerObj):
    ...
    for idx, dataset in enumerate(glob.glob(f'{inputPath}/*.h5ad')):
        ...
        h5ad_file = "/data"+str(RunnerObj.inputDir).split(str(Path.cwd()))[1]+"/CCMNET/" + Path(dataset).name
        params = "--skip_filtering" if str(RunnerObj.params['skip_filtering']) == 'True' else ""
        cmdToRun = " ".join(
            [
...
def parseOutput(RunnerObj):
    ...
    FinalDF = outDF[outDF['EdgeWeight'] == outDF.groupby(['Gene1','Gene2'])['EdgeWeight'].transform('max')].copy()
